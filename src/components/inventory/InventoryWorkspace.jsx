...
import {
  ...
  Building, ShieldCheck, Tag, DollarSign, Activity, FileSpreadsheet, BookmarkCheck, Truck
} from 'lucide-react';
...
export default function InventoryWorkspace({ isDarkMode }) {
  ...
  const [activeTab, setActiveTab] = useState('dashboard'); // 'dashboard' | 'items' | 'locations' | 'stock' | 'reservations' | 'fulfilments' | 'transactions'
  ...
  const [selectedReservationToIssue, setSelectedReservationToIssue] = useState(null);
    ...
    <div className={`flex-1 flex flex-col h-full overflow-hidden ${tBg}`}>
      ...
      <div className={`px-6 py-4 border-b flex flex-col sm:flex-row sm:items-center justify-between gap-4 shrink-0 ${isDarkMode ? 'border-slate-800 bg-slate-900/60' : 'border-slate-200 bg-white'}`}>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            onClick={() => {
              const activeRes = reservations.find(r => (r.status || 'active').toLowerCase() === 'active' || (r.status || '').toLowerCase() === 'partially_released');
              if (activeRes) {
                setSelectedReservationToIssue(activeRes);
            ...
            className="px-3.5 py-2 text-xs font-semibold rounded-lg bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 border border-emerald-500/30 transition-colors flex items-center gap-1.5"
          >
            <Truck className="w-3.5 h-3.5" /> Issue / Dispatch
          </button>
      ...
      <div className={`px-6 border-b flex items-center gap-6 overflow-x-auto shrink-0 ${isDarkMode ? 'border-slate-800 bg-slate-950' : 'border-slate-200 bg-slate-50'}`}>
        {[
          ...
          { id: 'fulfilments', label: 'Fulfilment & Dispatch', icon: Truck, count: fulfilments.length },
      ...
      <div className="flex-1 p-6 overflow-y-auto custom-scrollbar">
                      ...
                      .map(res => {
                        ...
                        return (
                          <tr key={res.id} className={isDarkMode ? 'hover:bg-slate-800/40' : 'hover:bg-slate-50'}>
                            ...
                            <td className="p-3.5 text-center">
                              {totalRem > 0 && statusLower !== 'released' && statusLower !== 'cancelled' && statusLower !== 'fulfilled' ? (
                                <div className="flex items-center justify-center gap-1.5">
                                  <button
                                    onClick={() => {
                                      setSelectedReservationToIssue(res);
                                      setIsIssueModalOpen(true);
                                    }}
                                    className="px-2.5 py-1 text-[11px] font-bold rounded bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 border border-emerald-500/30 transition-colors flex items-center gap-1"
                                    title="Issue / Dispatch physical stock"
                                  >
                                    <Truck className="w-3 h-3" /> Issue
                                  </button>
                                  <button
                                    onClick={() => {
                                    ...
                                    title="Release reservation back to available stock"
                                  >
                                    Release
                                  </button>
                                </div>
        ...
        {/* 6. FULFILMENT & DISPATCH HISTORY TAB */}
        {activeTab === 'fulfilments' && (
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <div>
                <h3 className="text-sm font-bold">Sales Fulfilment & Stock Issue Tracking</h3>
                <p className={`text-xs ${tMuted}`}>
                  Commercial Lifecycle: <span className="font-mono text-amber-400">ORDERED → RESERVED → ISSUED → REMAINING</span>
                </p>
              </div>
            </div>
            ...
            <div className={`rounded-xl border overflow-hidden ${tCard}`}>
              <table className="w-full text-left text-xs">
                <thead className={isDarkMode ? 'bg-slate-900 text-slate-400 border-b border-slate-800' : 'bg-slate-100 text-slate-700 font-semibold border-b border-slate-200'}>
                  <tr>
                    <th className="p-3.5">Sales Order / Customer</th>
                    <th className="p-3.5">Inventory Item</th>
                    <th className="p-3.5">Location & Lot</th>
                    <th className="p-3.5 text-right">Ordered Qty</th>
                    <th className="p-3.5 text-right">Reserved Qty</th>
                    <th className="p-3.5 text-right text-emerald-400">Issued Qty</th>
                    <th className="p-3.5 text-right text-amber-400">Remaining Reserved</th>
                    <th className="p-3.5">Fulfilment Progress</th>
                    <th className="p-3.5 text-center">Status</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-800/50">
                  {fulfilments.length === 0 && reservations.length === 0 ? (
                    <tr>
                      <td colSpan={9} className="p-8 text-center text-slate-500 text-xs">
                        No fulfilment or dispatch activity recorded yet.
                      </td>
                    </tr>
                  ) : (
                    reservations.flatMap(r => (r.lines || []).map(l => ({ reservation: r, line: l }))).map(({ reservation: r, line: l }, idx) => {
                      const reqQty = Number(l.requested_quantity || l.sales_order_item?.quantity || l.reserved_quantity || l.quantity) || 0;
                      const resQty = Number(l.reserved_quantity || l.quantity) || 0;
                      ...
                      const pct = reqQty > 0 ? Math.min(100, Math.round((issQty / reqQty) * 100)) : 0;
                      const status = (l.status || r.status || 'active').toLowerCase();

                      return (
                        <tr key={l.id || idx} className={isDarkMode ? 'hover:bg-slate-800/40' : 'hover:bg-slate-50'}>
                          <td className="p-3.5 font-medium">
                            <p className="font-semibold text-slate-100">{r.sales_order?.sales_order_number || r.sales_order?.order_number || 'Sales Order'}</p>
                            <p className={`text-[11px] ${tMuted}`}>{r.sales_order?.customer_name || 'Customer'}</p>
                          </td>

                          <td className="p-3.5">
                            <p className="font-semibold text-slate-200">{l.item?.name || l.sales_order_item?.item_description || 'Inventory Item'}</p>
                            <p className={`text-[10px] font-mono ${tMuted}`}>{l.item?.item_code || ''}</p>
                          </td>

                          <td className="p-3.5 font-mono text-slate-300 text-[11px]">
                            {l.location?.code ? `${l.location.code}` : 'Warehouse'}
                            {l.lot_number ? ` • Lot: ${l.lot_number}` : ''}
                          </td>

                          <td className="p-3.5 text-right font-mono font-bold text-slate-300">{reqQty}</td>
                          <td className="p-3.5 text-right font-mono font-bold text-blue-400">{resQty}</td>
                          <td className="p-3.5 text-right font-mono font-bold text-emerald-400">{issQty}</td>
                          <td className="p-3.5 text-right font-mono font-bold text-amber-400">{remQty}</td>
                          ...
                          <td className="p-3.5">
                            ...
                            <span className="text-[10px] font-mono text-slate-400 mt-1 block text-right">{pct}% Issued</span>
                          </td>

                          <td className="p-3.5 text-center">
                            <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase ${
                              issQty >= reqQty || status === 'fulfilled' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' :
                              issQty > 0 ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30' :
                              status === 'released' ? 'bg-slate-800 text-slate-400' :
                              'bg-amber-500/20 text-amber-400 border border-amber-500/30'
                            }`}>
                              {issQty >= reqQty || status === 'fulfilled' ? 'Fulfilled' : issQty > 0 ? 'Partially Issued' : status.replace('_', ' ')}
                            </span>
                          </td>
                        </tr>
                      );
