-- Migration: Procurement -> Accounts Purchase Bill Matching Hardening v3
-- Hardens 3-way matching in create_purchase_bill_with_accounting_atomic for GRNI clearing, price variance, and over-clearance protection
...
BEGIN
    ...
    -- Calculate Outstanding GRNI for PO matching to prevent over-clearing
    IF v_po_id IS NOT NULL THEN
        SELECT COALESCE(SUM(gni.line_total), 0) INTO v_outstanding_grni
        FROM public.goods_received_notes gn
        JOIN public.goods_received_note_items gni ON gni.grn_id = gn.id
        WHERE gn.purchase_order_id = v_po_id AND gn.tenant_company_id = v_op_co_id;

        v_grni_clearing_amount := LEAST(v_subtotal, v_outstanding_grni);
        v_ppv_amount := v_subtotal - v_grni_clearing_amount;
    ELSE
        v_grni_clearing_amount := 0;
        v_ppv_amount := 0;
    END IF;
    ...
    -- Insert Bill Items with item_code
    ...
    IF v_total_amount > 0 THEN
        ...
        IF v_po_id IS NOT NULL AND v_grni_acc_id IS NOT NULL AND v_grni_clearing_amount > 0 THEN
